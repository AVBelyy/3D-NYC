#!/usr/bin/env python3
"""Shared implementation for the NYC land-cover raster caches.

Both published collections carry the same eight-class legend on the same
six-inch grid in EPSG:2263, so one builder serves them and the generator can
read either through an identical cached layout.  What differs is only how the
survey was shipped: 2017 as an ERDAS IMG inside a ZIP, 2021 as a plain
GeoTIFF.  That difference is declared in ``COLLECTIONS`` and nowhere else.
"""
...
# The class values the generator relies on, shared by both collections:
# 1 tree canopy, 2 grass/shrub, 3 bare soil, 4 water, 5 building, 6 road,
# 7 other impervious, 8 railroad.  See scripts/_surface_styles.py for the only
# two the printable surfaces read.
LAND_COVER_CLASSES = {
    1: "tree canopy", 2: "grass/shrub", 3: "bare soil", 4: "water",
...
class Collection:
    """One published land-cover survey and the shape it arrives in."""

    name: str
    source: str
    # Member to read from inside the download, or None when the download is
    # itself the raster.  A ZIP is streamed through /vsizip rather than
    # extracted: the 2017 .img header is useless without its ~98 GB .ige
    # companion, and GDAL can read both members in place.
    member: str | None
    summary: str


COLLECTIONS = {
    "nyc_land_cover_2017": Collection(
        name="nyc_land_cover_2017",
        source="nyc_land_cover_2017/Land_Cover.zip",
        member="Land_Cover/NYC_2017_LiDAR_LandCover.img",
        summary="NYC 2017 six-inch land cover, published by the City of New York",
    ),
    "nyc_land_cover_2021": Collection(
        name="nyc_land_cover_2021",
        source="nyc_land_cover_2021/landcover_nyc_2021_6in.tif",
        member=None,
        summary="NYC 2021 six-inch land cover, published by TNC/UVM on Zenodo",
    ),
}
# The newest survey is what the generator and the planner read unless told
# otherwise.  The order is oldest-last so callers can take index 0 as default.
LAND_COVER_DATASETS = ("nyc_land_cover_2021", "nyc_land_cover_2017")


def gdal_dataset(collection: Collection, path: Path) -> str:
    """The GDAL dataset string for one downloaded source file."""
    if collection.member and path.suffix.lower() == ".zip":
        return f"/vsizip/{path}/{collection.member}"
    return str(path)


def default_source(collection: Collection) -> Path:
    return (DATA / "raw" / collection.source).resolve()


def parser(collection: Collection) -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(
        description=f"Cache {collection.summary} as a resumable native-resolution GeoTIFF."
    )
    value.add_argument(
        "--source", type=Path,
        help=f"Published source raster (default data/raw/{collection.source})",
    )
...
def resolve_source(collection: Collection, value: Path | None) -> tuple[Path, str]:
    path = value.resolve() if value is not None else default_source(collection)
    if not path.is_file():
        raise FileNotFoundError(
            f"{path} is missing; fetch it with scripts/download_{collection.name}.py"
        )
    return path, gdal_dataset(collection, path)
...
def build(collection: Collection, args: argparse.Namespace) -> None:
    if args.band_rows <= 0 or args.checkpoint_bands <= 0:
        raise ValueError("--band-rows and --checkpoint-bands must be positive")
    source_file, dataset = resolve_source(collection, args.source)
    bounds = parse_bounds(args.bounds)
    component_dir = args.cache_root.resolve() / collection.name
    component_dir.mkdir(parents=True, exist_ok=True)
    sources = {"landcover": file_signature(source_file, with_hash=args.hash_source)}
    configuration = {
        "pipeline_version": PIPELINE_VERSION,
        "cache_format_version": CACHE_FORMAT_VERSION,
        "source_member": collection.member if source_file.suffix.lower() == ".zip" else None,
    ...
    manifest = start_manifest(component_dir, collection.name, configuration, sources)
    ...
    # Both surveys publish the same legend. A value outside it means the source
    # is not the raster this cache claims to hold, and every downstream reader
    # would silently treat the surplus class as unvegetated built ground.
    unexpected = sorted(set(counts) - set(LAND_COVER_CLASSES) - {0})
    if unexpected:
        raise ValueError(
            f"{dataset} carries land-cover classes outside the published legend: "
            f"{unexpected}. Expected {sorted(LAND_COVER_CLASSES)}."
        )
...
def main_for(name: str) -> None:
    ...
    build(collection, parser(collection).parse_args())
