...
`data/cache/` is also ignored because every dataset is reproducible from its
matching scripts. The checked-in inputs are `bambu/project_settings.json` and
the reusable crop polygons in `polygons/`.
...
can alternatively stream the official feature service directly).
