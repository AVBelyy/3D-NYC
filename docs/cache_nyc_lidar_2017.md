...
`scripts/cache_all.sh` split the work for this collection. Both scripts take
this path only when given `--lidar-dataset nyc_lidar_2017`; `nyc_lidar_2021`
is the default, so pass the same name to each.
Omitting `--bounds` from the download command instead selects every indexed
tile, so reach for it only when every LAZ really does belong on disk.
